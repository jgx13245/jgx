...
<template>
	<div class="position" v-show="show" ref="mmm">
		...
		<div class="your-position">
			<div class="CityChange-main-title">{{word}}热门城市</div>
			<div class="your-position-now">
				<div class="your-position-now-name" v-for="item in ChinahotCity" :key="item">
					<a>{{item}}</a>
				</div>
			</div>
		</div>
		<ul>
			<li v-for="item in China" :key="item.id"  class="cityPosition" >
...
export default {	
		...
		computed: mapState({
			...
			ChinahotCity(state){
					return state.selectCity.ChinahotCity
			}
		}),
